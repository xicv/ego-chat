...
Use durable convergence whenever a Codex request explicitly requires multiple exchanges until settlement, and use it for a deliberately separate broker-owned implementation task. Codex can call `ego_start_convergence` and later `await_workflow`, or call `ego_converge_until_settled` with either progress notifications or `waitMode: token_saver`. Closing the MCP facade only detaches that waiter; the daemon keeps alternating the dedicated Codex task and the same ChatGPT conversation. The broker requires observable workspace-capable App Server activity before it accepts a Codex candidate for external review. If Codex emits a result without inspecting the workspace or returns a semantically inconsistent candidate, the broker keeps correcting that same task with backoff. If App Server exits during setup or any accepted turn, the broker reconnects, resumes the exact task and turn, recovers any completed result, and continues without a fixed exit ceiling. A Codex `blocked` result is sent to ChatGPT as review evidence rather than terminalizing convergence.

...
`wallClockTimeoutMs` is a host attachment window, not a workflow kill switch, and may span up to eight hours. Codex and ZCode setup configure a matching eight-hour-plus-five-minute MCP tool cap, leaving transport margin outside the broker window while the one-minute heartbeat keeps the attached call observable. Per-review and per-Codex-turn deadlines remain recovery triggers, not overall workflow ceilings. Omitting `maxCycles` means no review-cycle ceiling. A fully exited current-host Codex or ZCode task still cannot be externally awakened by Ego Chat, so explicit until-settled Codex requests default to detached convergence with a broker-owned Codex App Server task.

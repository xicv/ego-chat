...
Ego Chat is designed as one durable conversation manager, not a sequence of one-shot browser scripts. Codex, ZCode, or Claude Code remains side A, one persistent ChatGPT conversation in Ego Browser remains side B, and review feedback returns directly to side A until the target is settled.
...
After Send, the broker performs read-only capture and reconciliation. It keeps the same workflow alive until it can attribute the marked user/assistant pair or prove the marked prompt absent. Only proven absence permits a uniquely marked new delivery attempt. A restart may clear a composer draft only when its canonical digest and unique marker prove it is the exact unsent Ego Chat prompt; unrelated human drafts remain untouched. Provider terminal banners (stopped, conversation exhaustion, quota, or a provider error) are matched by an exact sentence first and, failing that, by a same-node token-pair rule under fixed precedence; a pending, non-terminal capture also retains up to four visible on-page status labels as read-only diagnostic evidence.

The exact deterministic task space belongs to its binding and is reclaimed automatically for Send and recovery. When it has vanished and Ego Chat itself named it, the broker recreates it by name after a short delay and reopens the conversation in it. Using another Ego Space does not interrupt the workflow. Independent bindings queue through the broker's shared browser lane rather than racing Ego's global automation channel. A create-once conversation whose confirmed send carries this same recreatable identity recovers exactly like a bound conversation, and a send left stuck on that vanished-space evidence remains reconcilable: reconciliation recreates the vanished Space by name and reopens the confirmed conversation (no wait, since the workflow is already stopped) before reading its attributable response, without a second Send.
...
By default, attributed conversation exhaustion durably checkpoints the candidate and its exact source evidence, then hands off to one isolated same-project successor rather than stopping. The successor's first review prompt carries a bounded, redacted summary of the exhausted conversation's prior review as explicitly untrusted context. That first review's turn/terminal identity is exact and unique; when it stops on a proven pre-Send driver failure (no Send click started, no draft cleared), the broker reissues it exactly once under a distinct identity before falling back to a retained, resumable pause — a second such failure, or any other failure, pauses immediately. Restart always rediscovers whichever attempt is durably in flight and never starts a third. A paused convergence's public record exposes what candidate it is holding (status, a bounded summary, per-criterion status, a blocker count) without exposing the private review packet or per-criterion evidence text.
...
`wallClockTimeoutMs` is a host attachment window, not a workflow kill switch, and may span up to eight hours. Codex, ZCode, and Claude Code setup configure a matching eight-hour-plus-five-minute MCP tool cap, leaving transport margin outside the broker window while the one-minute heartbeat keeps the attached call observable. Per-review and per-Codex-turn deadlines remain recovery triggers, not overall workflow ceilings. Omitting `maxCycles` means no review-cycle ceiling. A fully exited current-host Codex, ZCode, or Claude Code task still cannot be externally awakened by Ego Chat, so explicit until-settled Codex requests default to detached convergence with a broker-owned Codex App Server task.

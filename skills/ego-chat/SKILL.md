...
Keep the default `waitMode: progress` for durable convergence so the user sees deterministic phase changes, recovery counters, and whether the current ChatGPT review is not started, unconfirmed, durably sent, or captured. This supervisor uses local broker and browser-delivery state; it does not invoke a model or add a second workflow. Use `waitMode: token_saver` only when the user explicitly asks for a silent wait. Keep either single MCP call pending; do not poll, emit speculative waiting commentary, or start a second workflow while ChatGPT thinks. If the connected host loses only the waiter and receives a durable workflow ID, reattach to that workflow rather than resending.

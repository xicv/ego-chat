...
- `lastDowngrade` is populated when a captured answering model ranked below the last known-good one for its binding: the broker always dispatches a local `model_downgrade` alert for this, and a convergence started with `answeringModelPolicy: "pause"` additionally ends that cycle as `human_required` with code `answering_model_downgraded` and retains the downgraded review without resending it; the default `"alert"` policy only notifies and the cycle continues.
...
- After starting an until-settled convergence (`ego_start_convergence` or `ego_converge_until_settled`) from a host with a shell, run the returned `supervision.monitorCommand` once — it is idempotent — to attach Eagle Monitor, and run `eagle-monitor stop --json` after settlement or cancellation. Never start the monitor for a single exchange (`ego_exchange_and_wait`, `ego_review_candidate_and_wait`); it has no `supervision.monitorCommand`.
...
For an unattended task that may outgrow its chat, the default is automatic: set `conversationContinuation: "manual"` when starting convergence to opt out. Only attributed `chatgpt_conversation_exhausted` evidence permits the broker to prepare one isolated same-project successor, review the exact retained candidate using the enforced strongest-model/maximum-effort policy, and promote its verified permanent URL/head. Attach the same running parent through `successor_preparing` and `successor_reviewing`; never send a handoff yourself. Restart rediscovers the same child operation. A failed first successor remains reserved for exact recovery, not another replacement.

The successor's first review prompt carries a bounded, redacted summary of the exhausted conversation's prior review as explicitly untrusted context, so the new chat is not starting cold. If that first review stops on a proven pre-Send driver failure (no Send click started, no draft cleared), the broker retries it exactly once under a distinct turn/terminal identity before falling back to today's pause; any other failure, or a second failure, pauses immediately with the checkpoint retained. A paused convergence's public record carries `candidateSummary` (status, a bounded summary, per-criterion status, and a blocker count) alongside `continuationCheckpoint` so a host can see what candidate is being held without exposing the private review packet or per-criterion evidence text.

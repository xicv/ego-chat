...
Supply an immutable target, acceptance criteria, absolute `cwd`, and the least-permissive sandbox. Use `workspace-write` when the user authorized local implementation or review fixes; use `read-only` for review-only work. Omit `maxCycles` for until-settled behavior. `wallClockTimeoutMs` bounds one host attachment window at eight hours; setup configures Codex and ZCode host tool caps five minutes longer. `chatGptTimeoutMs` and `codexTurnTimeoutMs` remain per-operation recovery triggers, not overall workflow ceilings.

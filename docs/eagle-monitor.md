...
A pre-Send stall escalates: `stalled_before_send` stays `humanRequired: false` (reason `pre_send_progress_stalled`, action set unchanged) until the workflow age reaches `preSendStallEscalationMs` (30 minutes), after which it reports `humanRequired: true` with reason `pre_send_stall_escalated` and gains `notify_user`. `healthy` also gains `notify_user`: in safe mode, when the operational state is `healthy` or `send_confirmed_capture` and the semantic classification is `stagnant` or `looping`, the engine submits one notification (reason `semantic_stagnant` or `semantic_looping`, `state` equal to the operational state), deduplicated against the retained incidents by the semantic incident key rather than the repeat-timer state below — a state transition back to `healthy` clears the plain human-required notification slot every tick, so a semantic notification is one-shot per incident key, not repeated. A `suspect` semantic classification never notifies. Shadow mode never submits any notification (`predicted_shadow_only`).

An accepted human-required notification (the plain, non-semantic path) repeats while the same incident (identity plus reason code plus state) stays open: the first repeat is due `notificationRepeatMs` (60 minutes) after acceptance, and each subsequent repeat doubles that delay, capped at `notificationRepeatMaxMs` (4 hours). The repeat resets when the incident's identity changes or the state returns to `healthy` or `settled`. `status` and `incidents` expose the current attempt as `notification.repeatCount`.

`eagle-monitor start` and `eagle-monitor status --json` report `notifications: "local"` in safe mode and `notifications: "suppressed_in_shadow_mode"` in shadow mode.

`POLICY_CONTRACT`'s digest changed to add `preSendStallEscalationMs`, `notificationRepeatMs`, and `notificationRepeatMaxMs`. A monitor session already running against the previous digest reports `version_skew` (reason `monitor_policy_version_skew`) until it is stopped and started again; this is deliberate, since the policy governs which actions are allowed in which state.

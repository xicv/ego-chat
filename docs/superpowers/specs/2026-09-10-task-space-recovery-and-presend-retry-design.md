...
4. Persistence and visibility.
   ...
   - Capture path: the first live observation after an outage (pending or final) writes `exchange.task_space_restored`, which persists any recreation record (`private.captureTaskSpaceRecovery` plus the public field) and stamps `resolvedAt` on the `bound_task_space_missing` record, so a pending capture cannot drop the evidence and a later disappearance starts its own outage and its own delay. The final capture result then carries the capture record, or else the send's, on `result.taskSpaceRecovery`, and `exchange.response_captured` updates the workflow field.

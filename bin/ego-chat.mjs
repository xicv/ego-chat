...
try {
  if (command === "ping") {
    ...
    if (args.length !== 1) {
      failUsage("capture-attachment requires one JSON input file")
    }
  ...
  } else if (command === "release-attachment-evidence") {
    if (args.length !== 1) {
      failUsage("release-attachment-evidence requires one JSON input file")
    }
    const input = JSON.parse(await fs.readFile(args[0], "utf8"))
    print(await requestBroker(config, "attachment.release", input))
